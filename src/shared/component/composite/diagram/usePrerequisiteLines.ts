...
import type { PrerequisiteEdge } from "./prerequisiteDiagram";
...
export function getPrerequisiteLinePath(
  ...
  if (
    Math.abs(courseCenterX - prerequisiteCenterX) >=
    Math.abs(courseCenterY - prerequisiteCenterY)
  ) {
    const pointsRight = prerequisiteCenterX <= courseCenterX;
    const startX =
      (pointsRight ? prerequisite.right : prerequisite.left) - root.left;
  ...
  const startY =
    (pointsDown ? prerequisite.bottom : prerequisite.top) - root.top;
