...
import {
  PROFESSOR_COLLECTION_VIEW_CONFIG,
  type ProfessorCollectionFieldId,
} from "./professorCollectionViewConfig";
import type { ProfessorCollectionDto } from "@/external/dto/professor/professor.dto";
...
const cellRenderers = {
  ...
  status: ({ row }) => {
    const active =
      ...
      row.original.status.toLowerCase() === "activo";
  ...
  ProfessorCollectionItem,
  ProfessorCollectionFieldId
>;

const getStudentGradeRowId: GetItemId<ProfessorCollectionDto> = (row) => row.id;
export function useProfessorCollectionTable(
  data: readonly ProfessorCollectionItem[],
) {
  const query = useDataSearchQuery();
  const { setConditions, setSearchText } = useDataSearchActions();
  const { globalFilter, setGlobalFilter, table, filterResult, metadata } =
    useTable({
