...
const CONFIG = {
  fields: [
    ...
    },
    {
      ...
      icon: "toolOutline",
      valueType: "text",
      accessor: (item) => item.equipments.join(", "),
      format: (item) => item.equipments.join(", ") || "--",
    },
    {
      fieldId: "admin",
      label: "Administrador",
      icon: "admin",
      valueType: "text",
      accessor: (item) => item.admin,
      format: (item) => item.admin || "--",
    },
  ],
...
export function ClassroomCollection({ activeId }: { activeId?: string }) {
  const snapshot = useLiveQuery(async () => {
    try {
      return { data: await fetchClassroomCollection() } as const;
    } catch (error) {
      console.error("Failed loading classrooms", error);
  ...
  return (
    <ClassroomCollectionPresenter
      activeId={activeId}
      data={data}
      error={Boolean(snapshot && "error" in snapshot)}
      loading={!snapshot}
      metadata={metadata}
      onClose={navigation.closeClassroomPreview}
      onOpen={navigation.openClassroomDetail}
      onSearchTextChange={setGlobalFilter}
      onSelect={navigation.selectClassroom}
      renderPreview={(id) => (
        <ClassroomDetail className="px-4" classroomId={id} />
      )}
