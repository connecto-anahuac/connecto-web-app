...
import FileCardViewSub from "./FileCardView";
import FileCardView from "./FileCardView2";
...
export default function FileSelectorPanel({ className }: Props) {
  ...
  return (
    <div
      className={cn(
        "p-5 flex flex-col gap-4 w-104 h-full rounded-lg bg-SurfaceContainerLowest",
        className,
      )}
    >
      ...
      >
        {/* 💡 条件分岐: ファイルが選択されているかどうかで表示を切り替える */}
        {sources.length > 0 ? (
          <div className="flex flex-col justify-start p-2 h-full gap-2 w-full overflow-y-auto">
      ...
      <button
        className={cn(
          "text-sm w-full py-3 rounded-sm flex justify-center items-center font-medium text-OnPrimary bg-Primary cursor-pointer",
          sources.length === 0 && " bg-[#E6E6E6] text-[#616161]  cursor-not-allowed",
        )}
        onClick={handleFile}
