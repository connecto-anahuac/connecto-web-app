...
const plans: OfferingCoursePanelPlan[] = [
  { id: "tind", label: "TIND", semesters: [{ id: "semester-8", label: "Semestre 8", expectedStudents: [{ id: "student-1", fullName: "Rayan Garcia Reyes", avatarColor: "var(--CUL-strong)" }, { id: "student-2", fullName: "Ana Sofía Martínez", avatarColor: "var(--ADM-strong)" }], studentsWithoutPrerequisites: [{ id: "student-3", fullName: "Daniel Torres", avatarColor: "var(--SIS-strong)" }] }] },
  { id: "industrial", label: "Industrial", semesters: [{ id: "semester-6", label: "Semestre 6", expectedStudents: [{ id: "student-2", fullName: "Ana Sofía Martínez" }], isMulti: true }] },
];
...
export const Interactive: Story = { play: async ({ args, canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("tab", { name: "Industrial" })); await expect(args.onSelectedPlanChange).toHaveBeenCalledWith("industrial"); await userEvent.click(canvas.getByRole("button", { name: "Increase sessions" })); await expect(args.onSessionCountChange).toHaveBeenCalledWith(2); await userEvent.click(canvas.getByRole("button", { name: "Close offering course panel" })); await expect(args.onClose).toHaveBeenCalled(); } };
