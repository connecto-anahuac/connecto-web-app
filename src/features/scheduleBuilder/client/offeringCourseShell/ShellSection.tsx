...
import type { SearchTool } from "@/shared/component/composite/datasection/buttonmodal/type";
...
export default function ShellSection<TItem>({
  ...
  const tableState = table && typeof table.getState === "function"
    ? table.getState()
    : { sorting: [], columnPinning: { left: [] } };
  ...
  return (
    ...
    >
      <div
        className={cn("relative flex flex-col gap-2 min-h-0 flex-1 max-w-full", className)}
        ...
        <div className="flex gap-4 items-end">
                ...
                <div className=" flex min-w-72 w-fit flex-col gap-2 rounded-md border border-Outline bg-InverseSurface p-2 text-InverseOnSurface shadow-lg">
                  ...
                  <div className="border-t border-Outline pt-1">
                    {tableConfig!.fields
                      .filter(
                        (column) =>
                          !tableState.sorting.some((sort) => sort.id === column.fieldId),
                      )
                      .map((column) => (
                        <button
          ...
          {isToolEnable("pivot") && (
            <PivotButtonModal
              ...
              tableConfig={tableConfig}
              hasBadge={
                (tableState.columnPinning?.left?.length ?? 0) > 0
              }
            />
          )}
