...
description: "Manage Next.js App Router architecture work in this repository. Use when: route changes, feature slice work, data fetching, external layer changes, DTO updates, validation, architecture-aware refactors, or finding and loading the Next.js skills under .agents/skills."
argument-hint: "Describe the Next.js implementation, refactor, routing, data-flow, external-layer, DTO, or validation task."
---


# Next.js Implementation Manager

## description
Manage Next.js App Router architecture work in this repository. Use when: route changes, feature slice work, data fetching, external layer changes, DTO updates, validation, architecture-aware refactors, or finding and loading the Next.js skills.
...
- For end-to-end feature implementation across multiple layers, start with `.agents/skills/nextjs-app-router-architecture/SKILL.md`.
- For `page.tsx`, `layout.tsx`, route groups, `loading.tsx`, `error.tsx`, typed routes, metadata, or `PageProps` / `LayoutProps`, load `.agents/skills/nextjs-arch-routing/SKILL.md`.
- For `features/**`, Container / Presenter / Hook structure, feature types, colocated tests, or local query helpers, load `.agents/skills/nextjs-arch-feature-slice/SKILL.md`.
- For hydration, TanStack Query, query keys, prefetch, invalidation, server-first fetching, or mutation flow, load `.agents/skills/nextjs-arch-data-flow/SKILL.md`.
- For `external/**`, DTOs, handlers, services, repositories, business logic, validation helpers, or server-only boundaries, load `.agents/skills/nextjs-arch-external-layer/SKILL.md`.
- For lint, typegen, tests, format checks, route validation, feature validation, or change-scoped verification, load `.agents/skills/nextjs-arch-quality-gates/SKILL.md`.
- For document priority, architecture guide lookup, or deciding which architecture document is canonical, load `.agents/skills/nextjs-arch-doc-references/SKILL.md`.
...
2. Read only the matching skill `SKILL.md` files from `.agents/skills/`.
